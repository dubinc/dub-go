...
```go
dubgo.WithServerURL("https://api.example.com")
```
...
```go
dubgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
dubgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
dubgo.WithTimeout(30 * time.Second)
```
