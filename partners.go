...
func newPartners(sdkConfig sdkConfiguration) *Partners {
	return &Partners{
...
func (s *Partners) UpdateSale(ctx context.Context, request *operations.UpdatePartnerSaleRequestBody, opts ...operations.Option) (*operations.UpdatePartnerSaleResponseBody, error) {
	o := operations.Options{}
	supportedOptions := []string{
		operations.SupportedOptionRetries,
		operations.SupportedOptionTimeout,
	...
	opURL, err := url.JoinPath(baseURL, "/partners/sales")
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out operations.UpdatePartnerSaleResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
